...
from model_hub.config import ProviderConfigs, ModelConfig
...
class Prompter:
    ...
    def __init__(self, default_model: Optional[str] = None, provider_configs: Optional[ProviderConfigs] = None):
        """
        ...
        if not provider_configs:
            raise ValueError("You must provide atleast one model provider configuration.")
        
        # Initialize only providers with valid configs
        self._append_to_model_provider_list(provider_configs)
    
    def _append_to_model_provider_list(self, provider_configs: ProviderConfigs):
        for provider_name, config in provider_configs.items():
            if provider_name in self._provider_map and config is not None:
                provider_class = self._provider_map[provider_name]
                self._model_providers.append(provider_class(config))

    
    def set_providers(self, provider_configs: Optional[ProviderConfigs] = None):
        """ 
    ...
    def update_providers(self, provider_configs: Optional[ProviderConfigs] = None):
        """ 
        ...
        self._model_providers = [provider for provider in self._model_providers if provider.get_name().value not in provider_configs.keys()]
    ...
    def send(self, prompt: str, model: Optional[str] = None) -> str:
        ...
        if self._default_model is None and model is None:
            raise ValueError("Prompter default_model and model arg both None. Atleast one must be defined.")
